(function(global, factory) {
  ...
  define("deparam", function(params, primitive) {
    var obj = {};
    var primitive_types = {'true': !0, 'false': !1, 'null': null, 'undefined': undefined};
    ...
    params.replace(/\+/g, ' ').split('&').forEach(function(v, j){
      ...
      if(param.length === 2) {
        ...
        if(primitive) {
          ...
          else if(val in primitive_types) val = primitive_types[val];
        }
        if(keys_last) {
          ...
          else if(typeof obj[key] !== 'undefined') obj[key] = [obj[key], val];
          else obj[key] = val;
        }
      } 
      else if(key) obj[key] = primitive ? undefined : '';
    });
    return obj;
  });
...
console.log(Util.deparam('m=log'));