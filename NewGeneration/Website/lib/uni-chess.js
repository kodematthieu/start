(function(global, factory) {
  "use strict";
  if(typeof module === "object" && typeof module.exports === "object") {
    module.exports = global.document ? factory(global, true) : function(w) {
      if(!w.document) throw new Error("UniChess requires a window with a document to run");
      return factory(w);
    };
  }
  else factory(global);
})(typeof window !== "undefined" ? window : this, function(window, noGlobal) {
  "use strict";
  
  var HORIZONTAL = "abcdefgh".split("");
  var UUID = function() {return "xxxx-xxxxxxxx-xxxxxxxx-xxxx".replace(/x/g, () => Math.round(Math.random()*16).toString(16));};
  var MOVETOLOCATION = function(move) {
    var x, y;
    move = move.split("");
    for(var i = 0; i < HORIZONTAL.length; i++) if(HORIZONTAL[i] === move[0]) x = i;
    y = Math.abs(parseInt(move[1]) - 8);
    return {x,y};
  };
  var LOCATIONTOMOVE = function({x,y}) {
    x = HORIZONTAL[x];
    y = Math.abs(8 - y);
    return x+y;
  };
  var selectedPiece;
  
  var textures = {
    white: {
      pawn: "data:image/png;base64,",
      bishop: "data:image/png;base64,",
      knight: "data:image/png;base64,",
      rook: "data:image/png;base64,",
      queen: "data:image/png;base64,",
      king: "data:image/png;base64,",
    },
    black: {
      pawn: "data:image/png;base64,",
      bishop: "data:image/png;base64,",
      knight: "data:image/png;base64,",
      rook: "data:image/png;base64,",
      queen: "data:image/png;base64,",
      king: "data:image/png;base64,",
    }
  };
  
  var UniChess = {};
  UniChess.LOCATIONTOMOVE = LOCATIONTOMOVE;
  UniChess.MOVETOLOCATION = MOVETOLOCATION;
  
  // Board class in UniChess
  var Board = UniChess.Board = function Board(width, height, theme) {
    if(arguments.length < 2) throw new Error(`2 arguments are required, only ${arguments.length} are present`);
    if(typeof width !== "number") throw new TypeError("First argument (width) should be a number");
    if(typeof height !== "number") throw new TypeError("Second argument (height) should be a number");
    this.theme = Object.assign({light: "#f0d9b5", dark: "#b58863", highlight: "#ff0"}, typeof theme === "object" ? theme : {});
    this.size = {width,height};
    var size = [this.size.width/8, this.size.height/8];
    for(var j = 0; j < 8; j++) {
      var row = [];
      for(var i = 0; i < 8; i++) row.push({highlight: false, piece: null, type: j % 2 == i % 2 ? 'white' : 'black', x: i*size[0] + size[0]/2, y: j*size[1] + size[1]/2, render: { x: i*size[0], y: j*size[1], width: size[0] - 1, height: size[1] - 1}});
      [].push.call(this, row);
    }
  };
  Board.prototype.render = function(canvas, noLoop) {
    if(typeof canvas === "string") canvas = document.querySelector(canvas);
    if(canvas instanceof HTMLCanvasElement) canvas = canvas.getContext("2d");
    if(!(canvas instanceof CanvasRenderingContext2D)) throw new Error("Canvas is missing");
    var _this = this;
    canvas.canvas.addEventListener("click", choose);
    function choose(event) {
      var mouseX = event.offsetX;
      var mouseY = event.offsetY;
      var box = _this.size.width/8;
      for(var y = 0; y < 8; y++) {
        for(var x = 0; x < 8; x++) {
          if((mouseX < box*(x+1) && mouseX > box*x) && (mouseY < box*(y+1) && mouseY > box*y)) {
            if(!!_this.get(LOCATIONTOMOVE({x,y}))) {
              for(var i = 0; i < _this.get(LOCATIONTOMOVE({x,y})).getMoves().length; i++) {
                var pos = MOVETOLOCATION(_this.get(LOCATIONTOMOVE({x,y})).getMoves()[i]);
                _this[pos.y][pos.x].highlight = true;
              }
              selectedPiece = _this.get(LOCATIONTOMOVE({x,y}));
              canvas.canvas.removeEventListener("click", choose);
              canvas.canvas.addEventListener("click", move);
            }
          }
        }
      }
    }
    function move(event) {
      var mouseX = event.offsetX;
      var mouseY = event.offsetY;
      var box = _this.size.width/8;
      for(var y = 0; y < 8; y++) {
        for(var x = 0; x < 8; x++) {
          if((mouseX < box*(x+1) && mouseX > box*x) && (mouseY < box*(y+1) && mouseY > box*y)) {
            var validate = (_this.get(selectedPiece.location).getMoves().map(e => MOVETOLOCATION(e)).filter(e => e.x === x && e.y === y).length === 1);
            if(validate) _this.move(selectedPiece.location, LOCATIONTOMOVE({x,y}));
            else if(selectedPiece.location === LOCATIONTOMOVE({x,y}));
            for(var j = 0; j < 8; j++) {
              for(var i = 0; i < 8; i++) {
                _this[j][i].highlight = false;
              }
            }
            selectedPiece = null;
            canvas.canvas.removeEventListener("click", move);
            canvas.canvas.addEventListener("click", choose);
          }
        }
      }
    }
    var loop = () => {
      if(noLoop) requestAnimationFrame(loop);
      if(noLoop) canvas.clearRect(0, 0, this.size.width, this.size.height);
      for(var j = 0; j < 8; j++) {
        for(var i = 0; i < 8; i++) {
          var box = this[j][i];
          var line = Math.min(box.render.width, box.render.height) / 10;
          canvas.beginPath();
          if(box.type === "white") canvas.fillStyle = this.theme.light;
          else canvas.fillStyle = this.theme.dark;
          if(!!box.highlight) {
            canvas.strokeStyle = this.theme.highlight;
            canvas.lineWidth = line;
            canvas.rect(box.render.x + (((line/2)/5)*4), box.render.y + (((line/2)/5)*4), box.render.width - (line-1), box.render.height - (line-1));
          }
          else {
            canvas.strokeStyle = "#000";
            canvas.lineWidth = 1;
            canvas.rect(box.render.x, box.render.y, box.render.width, box.render.height);
          }
          canvas.stroke();
          canvas.fill();
          canvas.closePath();
          if(box.piece && box.piece.texture) canvas.drawImage(box.piece.texture, box.render.x, box.render.y, box.render.width, box.render.height);
        }
      }
    };
    loop();
  };
  Board.prototype.setup = function() {
    var black = {
      "Pawn": ["a7", "b7", "c7", "d7", "e7", "f7", "g7", "h7"],
      "Bishop": ["c8", "f8"],
      "Knight": ["b8", "g8"],
      "Rook": ["a8", "h8"],
      "Queen": ["d8"],
      "King": ["e8"]
    };
    var white = {
      "Pawn": ["a2", "b2", "c2", "d2", "e2", "f2", "g2", "h2"],
      "Bishop": ["c1", "f1"],
      "Knight": ["b1", "g1"],
      "Rook": ["a1", "h1"],
      "Queen": ["d1"],
      "King": ["e1"]
    };
    for(var [piece, moves] of Object.entries(black)) {
      for(var move of moves) {
        this.set(move, new UniChess[piece](false));
      }
    }
    for(var [piece, moves] of Object.entries(white)) {
      for(var move of moves) {
        this.set(move, new UniChess[piece](true));
      }
    }
  };
  Board.prototype.set = function(where, piece) {
    if(arguments.length < 2) throw new Error(`2 arguments are required, only ${arguments.length} are present`);
    if(typeof where !== "string") throw new TypeError("First argument (where) should be a string, eg. e4");
    if(!(piece instanceof Piece) && piece != null) throw new TypeError("Second argument (piece) should be an instance of UniChess.Piece");
    where = MOVETOLOCATION(where.match(/[a-h][1-8]/g)[0]);
    this[where.y][where.x].piece = piece;
    if(piece !== null) {
      piece.location = move;
      piece.board = this;
    }
    return this;
  };
  Board.prototype.get = function(where) {
    if(arguments.length < 1) throw new Error(`1 argument is required, only ${arguments.length} are present`);
    if(typeof where !== "string") throw new TypeError("First argument (where) should be a string, eg. e4");
    where = MOVETOLOCATION(where.match(/[a-h][1-8]/g)[0]);
    return this[where.y][where.x].piece;
  };
  Board.prototype.move = function(from, to) {
    if(arguments.length < 2) throw new Error(`2 arguments are required, only ${arguments.length} are present`);
    if(typeof from !== "string") throw new TypeError("First argument (from) should be a string, eg. e4");
    if(typeof to !== "string") throw new TypeError("Second argument (to) should be a string, eg. e4");
    if(!!this.get(from) && ((!!this.get(to) && this.get(to).type !== this.get(from).type) || this.get(to) === null)) {
      this.set(to, this.get(from));
      this.set(from, null);
      this.get(to).move();
    }
    return this;
  };
  
  // Piece class in UniChess
  var Piece = UniChess.Piece = function Piece(isLight) {
    if(arguments.length < 1) throw new Error(`1 argument is required, only ${arguments.length} are present`);
    if(typeof isLight !== "boolean") throw new TypeError("First argument (isLight) should be a boolean");
    this.type = isLight ? "white" : "black";
    this.texture = null;
    this.board = null;
    this.location = null;
    this.move = function() {};
  };
  Piece.prototype.getBoxMoves = function() {
    var location = MOVETOLOCATION(this.location);
    var moves = [];
    for(var j = 0; j < 3; j++) {
      for(var i = 0; i < 3; i++) {
        var x = location.x+(i-1), y = location.y+(j-1);
        moves.push({x,y});
      }
    }
    moves = moves.filter(x => !(x.x === location.x && x.y === location.y));
    moves = moves.filter(x => x.x >= 0 && x.x < 8);
    moves = moves.filter(x => x.y >= 0 && x.y < 8);
    moves = moves.filter(x => (this.board.get(LOCATIONTOMOVE(x)) || {}).type !== this.type);
    for(var i = 0; i < moves.length; i++) moves[i] = LOCATIONTOMOVE(moves[i]);
    return moves;
  };
  Piece.prototype.getPawnMoves = function() {
    var location = MOVETOLOCATION(this.location);
    var type = this.type;
    var board = this.board;
    var isMoved = !(type === "white" && location.y === 6) && !(type === "black" && location.y === 1);
    var moves = [];
    if(type === "white" && location.y > 0) {
      if(!board.get(LOCATIONTOMOVE({x: location.x, y: location.y-1}))) moves.push(LOCATIONTOMOVE({x: location.x, y: location.y-1}));
      if(location.x > 0 && board.get(LOCATIONTOMOVE({x: location.x-1, y: location.y-1})) && board.get(LOCATIONTOMOVE({x: location.x-1, y: location.y-1})).type !== this.type) moves.push(LOCATIONTOMOVE({x: location.x-1, y: location.y-1}));
      if(location.x < 7 && board.get(LOCATIONTOMOVE({x: location.x+1, y: location.y-1})) && board.get(LOCATIONTOMOVE({x: location.x+1, y: location.y-1})).type !== this.type) moves.push(LOCATIONTOMOVE({x: location.x+1, y: location.y-1}));
      if(!isMoved && !board.get(LOCATIONTOMOVE({x: location.x, y: location.y-2}))) moves.push(LOCATIONTOMOVE({x: location.x, y: location.y-2}));
    }
    if(type === "black" && location.y < 7) {
      if(!board.get(LOCATIONTOMOVE({x: location.x, y: location.y+1}))) moves.push(LOCATIONTOMOVE({x: location.x, y: location.y+1}));
      if(location.x > 0 && board.get(LOCATIONTOMOVE({x: location.x-1, y: location.y+1})) && board.get(LOCATIONTOMOVE({x: location.x-1, y: location.y+1})).type !== this.type) moves.push(LOCATIONTOMOVE({x: location.x-1, y: location.y-1}));
      if(location.x < 7 && board.get(LOCATIONTOMOVE({x: location.x+1, y: location.y+1})) && board.get(LOCATIONTOMOVE({x: location.x+1, y: location.y+1})).type !== this.type) moves.push(LOCATIONTOMOVE({x: location.x+1, y: location.y-1}));
      if(!isMoved && !board.get(LOCATIONTOMOVE({x: location.x, y: location.y+2}))) moves.push(LOCATIONTOMOVE({x: location.x, y: location.y+2}));
    }
    return moves;
  };
  Piece.prototype.getDiagonalMoves = function() {
    var location = MOVETOLOCATION(this.location);
    var moves = [];
    var board = this.board;
    var _this = this;
    var condition = function(pos) { return (board.get(LOCATIONTOMOVE(pos)) || {}).type !== _this.type; };
    var pos;
    // Top Left
    pos = {x: location.x - 1, y: location.y - 1};
    while(pos.x > -1 && pos.y > -1 && condition(pos)) {
      pos.x -= 1;
      pos.y -= 1;
      moves.push(LOCATIONTOMOVE({x: pos.x + 1, y: pos.y + 1}));
      if(!!board.get(LOCATIONTOMOVE({x: pos.x + 1, y: pos.y + 1})) && condition) break;
    }
    // Top Right
    pos = {x: location.x + 1, y: location.y - 1};
    while(pos.x < 8 && pos.y > -1 && condition(pos)) {
      pos.x += 1;
      pos.y -= 1;
      moves.push(LOCATIONTOMOVE({x: pos.x - 1, y: pos.y + 1}));
      if(!!board.get(LOCATIONTOMOVE({x: pos.x - 1, y: pos.y + 1})) && condition) break;
    }
    // Bottom Left
    pos = {x: location.x - 1, y: location.y + 1};
    while(pos.x > -1 && pos.y < 8 && condition(pos)) {
      pos.x -= 1;
      pos.y += 1;
      moves.push(LOCATIONTOMOVE({x: pos.x + 1, y: pos.y - 1}));
      if(!!board.get(LOCATIONTOMOVE({x: pos.x + 1, y: pos.y - 1})) && condition) break;
    }
    // Bottom Right
    pos = {x: location.x + 1, y: location.y + 1};
    while(pos.x < 8 && pos.y < 8 && condition(pos)) {
      pos.x += 1;
      pos.y += 1;
      moves.push(LOCATIONTOMOVE({x: pos.x - 1, y: pos.y - 1}));
      if(!!board.get(LOCATIONTOMOVE({x: pos.x - 1, y: pos.y - 1})) && condition) break;
    }
    return moves;
  };
  Piece.prototype.getCrossMoves = function() {
    var location = MOVETOLOCATION(this.location);
    var moves = [];
    var board = this.board;
    var _this = this;
    var condition = function(pos) { return (board.get(LOCATIONTOMOVE(pos)) || {}).type !== _this.type; };
    var pos;
    // Left
    pos = {x: location.x - 1, y: location.y};
    while(pos.x > -1 && condition(pos)) {
      pos.x -= 1;
      moves.push(LOCATIONTOMOVE({x: pos.x + 1, y: pos.y}));
      if(!!board.get(LOCATIONTOMOVE({x: pos.x + 1, y: pos.y})) && condition) break;
    }
    // Top
    pos = {x: location.x, y: location.y - 1};
    while(pos.y > -1 && condition(pos)) {
      pos.y -= 1;
      moves.push(LOCATIONTOMOVE({x: pos.x, y: pos.y + 1}));
      if(!!board.get(LOCATIONTOMOVE({x: pos.x, y: pos.y + 1})) && condition) break;
    }
    // Right
    pos = {x: location.x + 1, y: location.y};
    while(pos.x < 8 && condition(pos)) {
      pos.x += 1;
      moves.push(LOCATIONTOMOVE({x: pos.x - 1, y: pos.y}));
      if(!!board.get(LOCATIONTOMOVE({x: pos.x - 1, y: pos.y})) && condition) break;
    }
    // Bottom
    pos = {x: location.x, y: location.y + 1};
    while(pos.y < 8 && condition(pos)) {
      pos.y += 1;
      moves.push(LOCATIONTOMOVE({x: pos.x, y: pos.y - 1}));
      if(!!board.get(LOCATIONTOMOVE({x: pos.x, y: pos.y - 1})) && condition) break;
    }
    return moves;
  };
  Piece.prototype.getKnightMoves = function() {
    var location = MOVETOLOCATION(this.location);
    var moves = [];
    var board = this.board;
    var _this = this;
    var condition = function(pos) { return ((pos.x < 8 && pos.x > -1) && (pos.y < 8 && pos.y > -1)) && (board.get(LOCATIONTOMOVE(pos)) || {}).type !== _this.type; };
    if(condition({x: location.x - 2, y: location.y - 1})) moves.push(LOCATIONTOMOVE({x: location.x - 2, y: location.y - 1}));
    if(condition({x: location.x - 1, y: location.y - 2})) moves.push(LOCATIONTOMOVE({x: location.x - 1, y: location.y - 2}));
    if(condition({x: location.x + 1, y: location.y - 2})) moves.push(LOCATIONTOMOVE({x: location.x + 1, y: location.y - 2}));
    if(condition({x: location.x + 2, y: location.y - 1})) moves.push(LOCATIONTOMOVE({x: location.x + 2, y: location.y - 1}));
    if(condition({x: location.x + 2, y: location.y + 1})) moves.push(LOCATIONTOMOVE({x: location.x + 2, y: location.y + 1}));
    if(condition({x: location.x + 1, y: location.y + 2})) moves.push(LOCATIONTOMOVE({x: location.x + 1, y: location.y + 2}));
    if(condition({x: location.x - 1, y: location.y + 2})) moves.push(LOCATIONTOMOVE({x: location.x - 1, y: location.y + 2}));
    if(condition({x: location.x - 2, y: location.y + 1})) moves.push(LOCATIONTOMOVE({x: location.x - 2, y: location.y + 1}));
    return moves;
  };
  // Pawn piece in UniChess
  var Pawn = UniChess.Pawn = class extends Piece {
    constructor(isLight) {
      super(isLight);
      var img = new Image();
      img.src = textures[this.type]["pawn"];
      img.onload = () => this.texture = img;
      var _this = this;
      this.move = function() {
        var pos = MOVETOLOCATION(_this.location);
        if(pos.y === (_this.type === "white" ? 0 : 7)) {
          _this.board.set(_this.location, new UniChess.Queen(_this.type === "white" ? true : false));
        }
      };
    }
    onmove(f) {
      this.move = f
    }
    getMoves() {
      return this.getPawnMoves();
    }
  };
  var Bishop = UniChess.Bishop = class extends Piece {
    constructor(isLight) {
      super(isLight);
      var img = new Image();
      img.src = textures[this.type]["bishop"];
      img.onload = () => this.texture = img;
    }
    getMoves() {
      return this.getDiagonalMoves();
    }
  };
  var Knight = UniChess.Knight = class extends Piece {
    constructor(isLight) {
      super(isLight);
      var img = new Image();
      img.src = textures[this.type]["knight"];
      img.onload = () => this.texture = img;
    }
    getMoves() {
      return this.getKnightMoves();
    }
  };
  var Rook = UniChess.Rook = class extends Piece {
    constructor(isLight) {
      super(isLight);
      var img = new Image();
      img.src = textures[this.type]["rook"];
      img.onload = () => this.texture = img;
    }
    getMoves() {
      return this.getCrossMoves();
    }
  };
  var Queen = UniChess.Queen = class extends Piece {
    constructor(isLight) {
      super(isLight);
      var img = new Image();
      img.src = textures[this.type]["queen"];
      img.onload = () => this.texture = img;
    }
    getMoves() {
      return [].concat(this.getCrossMoves(), this.getDiagonalMoves());
    }
  };
  var King = UniChess.King = class extends Piece {
    constructor(isLight) {
      super(isLight);
      var img = new Image();
      img.src = textures[this.type]["king"];
      img.onload = () => this.texture = img;
    }
    getMoves() {
      return this.getBoxMoves();
    }
  };
  if(!noGlobal) window.UniChess = UniChess;
  return UniChess;
});
